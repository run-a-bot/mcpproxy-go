...
to a profile with --profile-pin; every pinned key is then limited to the
profile's servers and, optionally, to the specific MCP tools selected for each
server. Profiles therefore provide a reusable second layer of authorization
for API keys, beyond each key's own server scope and permission tier.
